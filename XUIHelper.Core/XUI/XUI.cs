...
namespace XUIHelper.Core
{
    public abstract class XUI : IXUI
    {
        ...
        // Round-trip preservation: XUR5 has a Flags bit indicating whether a CountHeader
        // section is present. The heuristic in ShouldWriteCountHeader is calibrated for
        // Xbox 360 dashboards and produces wrong answers for some Zune HD files, so
        // when round-tripping we encode the original flag as an attribute on the XUI
        // root and consume it on recompile.
        ...
        public virtual async Task<bool> TryReadAsync()
        {
            try
            {
                ...
                // Pick up the XUR5-CountHeader round-trip marker if the XUI was emitted
                // by our writer. Absent attribute → null → fall back to the heuristic.
                string? countHeaderAttr = document.Root.Attribute("xur5CountHeader")?.Value;
                if (countHeaderAttr == "1") HasCountHeader = true;
                else if (countHeaderAttr == "0") HasCountHeader = false;
        ...
        public async Task<bool> TryWriteAsync(XUObject rootObject)
        {
            try
            {
                ...
                // Preserve XUR5 CountHeader presence for byte-exact round-trip.
                if (HasCountHeader.HasValue)
                {
                    rootElement.SetAttributeValue("xur5CountHeader", HasCountHeader.Value ? "1" : "0");
                }
