        ...
        public static XUTimeline? TryReadTimeline(this XUI12 xui, XElement element, XUObject obj)
        {
            try
            {
                ...
                foreach (XElement keyframeElement in keyframeElements)
                {
                    ...
                    // Ease values are part of every keyframe's XUR encoding regardless of
                    // interpolation type. Read whatever is present in the XUI; missing
                    // elements default to 0 (matches the "all-zero, omit emission"
                    // contract on the writer side).
                    byte easeIn = (byte)Convert.ToInt32(keyframeElement.Element("EaseIn")?.Value ?? "0");
                    byte easeOut = (byte)Convert.ToInt32(keyframeElement.Element("EaseOut")?.Value ?? "0");
                    byte easeScale = (byte)Convert.ToInt32(keyframeElement.Element("EaseScale")?.Value ?? "0");
