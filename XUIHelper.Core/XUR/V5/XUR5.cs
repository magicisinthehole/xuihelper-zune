...
namespace XUIHelper.Core
{
    ...
    {
        // Round-trip override for the CountHeader flag. Set externally (e.g., by the
        // conversion driver) to honor the originating XUI's xur5CountHeader marker
        // instead of the heuristic in ShouldWriteCountHeader. Null = use heuristic.
        public bool? WriteCountHeaderOverride { get; set; }
        ...
        {
            // Round-trip path takes priority: if the originating XUI carried the
            // xur5CountHeader marker, honor it exactly.
            if (WriteCountHeaderOverride.HasValue) return WriteCountHeaderOverride.Value;

            // Heuristic for from-scratch authoring (no source XUR). Calibrated against
            // the 195-file Zune HD v4.5 corpus: total property-value storage
            // (static properties across all objects plus animated keyframe values)
            // perfectly partitions flag=0 (max 633) from flag=1 (min 768).
            // Boundary set midway in the gap.
            int totalPropValues = rootObject.GetTotalPropertiesCount() + rootObject.GetTotalKeyframePropertiesCount();
            return totalPropValues >= 700;
        }
