        ...
        private XUObject? TryReadObject(IXUR xur, BinaryReader reader, ref XUObject parentObject)
        {
            try
            {
                ...
                if((flags & 0x4) == 0x4)
                {
                    ...
                    {
                        long pp = reader.BaseStream.Position;
                        byte[] pk = new byte[16];
                        int g = reader.BaseStream.Read(pk, 0, 16);
                        reader.BaseStream.Position = pp;
                        string h = string.Join(" ", pk.Take(g).Select(b => b.ToString("X2")));
                        xur.Logger?.Here().Verbose("TIMELINE-PRE-PEEK pos={0:X8} bytes=[{1}]", pp, h);
                    }
        ...
        private List<XUProperty>? TryReadProperties(IXUR xur, BinaryReader reader, string className)
        {
            try
            {
                ...
                foreach (XUClass xuClass in classList)
                {
                    ...
                    int upper5 = (packedByte >> 3) & 0x1F;
                    xur.Logger?.Here().Verbose("Class has {0} property masks (upper5={1}).", propertyMasksCount, upper5);
                ...
                {
                    xur.Logger?.Here().Error("Property count mismatch for class {0}. Expected: {1}, Actual: {2}", className, propertiesCount, retProperties.Count);
                    return null;
                }
